    ...
    var body: some View {
        NavigationStack{
            List {
                ...
                Section("该职位的人员（\(zhiWu.renYuan?.count ?? 0)人）：", content: {
                    ForEach(zhiWu.renYuanArray, id: \.self, content: {
                        i in
...
//                        Text(.active == self.editMode?.wrappedValue ? "Done" : "Edit")
    ...
    private func editZhiWuAction() {
        if zhiWu.mingCheng != mingCheng {
            LiuShuiJiLu_RenYuan(newLiuShui: LiuShui(context: viewContext), renyuan: nil, zhiwu: zhiWu, xiangmu: nil, lieBie: "职务_更名", beizhu: nil, houbeizhu: nil, sanRenYuan: nil, sanWuZi: nil, sanXiangMu: nil, sanZhiWu: mingCheng, qianZhiWu: zhiWu.mingCheng ?? "", qianXiangMu: nil)
            zhiWu.mingCheng = mingCheng
        }
        if zhiWu.beiZhu != beiZhu {
            zhiWu.beiZhu = beiZhu
        }
        do{
...
//struct ZhiWuInFoView_Previews: PreviewProvider {
